...
- Fixes
  ...
  - clean epicvaultDB input numbers
...
- API
  ...
  - Implement the API switch.
    ...
    - Set to 0 for epicscan priority, 1 for epicvaultDB priority. Setting it in the
    ...
    api.setApiSwitch(0);
    api.sendAsset(config); // sendAsset, claimEpicPulse and doInvoke will default to use epiccscan first
    api.setApiSwitch(1);
    api.doInvoke(config); // This call will default to use epicvaultDB first
...
- Wallet
  ...
  const w1 = EpicVault.create.wallet();
...
- API
  ...
  - Add `api.getToken` which is a combination of `api.getTokenInfo` and
    ...
    call. This is exposed semantically as `EpicVault.get.token`.
  - Bugfix CoinMarketCap truncating prices to integers.
  - Bugfix doTransferToken sending epicpulse to wrong address and appending wrong item
...
- Balance as an ES6 class.
  ...
  - `verifyAssets` to validate unspent coins against a given EpicChain node. Used to
  ...
  balance.assetSymbols = ['XPR', 'XPP']
  // Lookup assets using their symbols
  balance.assets = {
    XPR: {balance: 1, unspent: [ Object ], spent: [], unconfirmed: []}
    XPP: {balance: 25.1, unspent: [ Object ], spent: [], unconfirmed: []}
  }
  ```
...
- Constants and util methods are now exported as
  ...
  EpicVault.CONST.DEFAULT_RPC;
  EpicVault.u.reverseHex;
...
- RPC
  ...
  - RPCClient class models a EpicChain Node. Instantiate with
    `EpicVault.create.rpcClient(url)`. Comes with built-in methods for RPC calls.
  - Query class models a RPC call. Instantiate with `EpicVault.create.query()`. Comes
    with built-in methods for RPC calls.

- API

  - epicvault-wallet-db API is shifted to `api` folder.
  - Added coinmarketcap query support for easy price queries.
  - Token query (XEP5) is shifted here.
  - EpicScan support added.
  - Hardware support integrated as external signingFunction provided as
    argument.
  - New core api methods: sendAsset, claimEpicPulse and doInvoke.
