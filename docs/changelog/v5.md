...
- tx
  - Update transaction attributes to EpicChain.
...
- rpc
  ...
  - Update default address version to match EpicChain.
- sc
...
- ledger
  - Updated to support EpicChain ledger app.
- rpc
...
- uri
  - Add new `epicvault-uri` package. `epicvault-uri` is a rename and upgrade of nep-9.
    Nep9 style uris are very specific to epicchain and are not too relevant in N3
    context. epicvault-uri retains the core support of token transfers while also
...
```
epicchain-cli: v3.0.0-rc2
epicvault-js: v5.0.0-next.10
```
...
```
epicchain-cli: v3.0.0-rc1
epicvault-js: v5.0.0-next.7
```
...
- Smart contract hashes are determined in a new way causing all native contract
  hashes to change.
- EpicChain address format has changed.
...
- sc
  - `EPICCHAIN_BINARY_*` and `EPICCHAIN_JSON_*` SYSCALLs removed.
  - Old `EPICCHAIN_CRYPTO_*` SYSCALLs removed. Introduced two new SYSCALLs
    `EPICCHAIN_CRYPTO_CHECKSIG` & `EPICCHAIN_CRYPTO_CHECKMULTISIG`.
...
```
epicchain-cli: v3.0.0-preview5
epicvault-js: v5.0.0-next.6
```
...
- More RPC methods results been streamlined to return base64 values requiring
  interface updates.
- EpicChain consumption results in RPC calls are no longer returned as decimal values.
...
- sc
  - EpicChain Executable Format (XEF) class has been added. This allows loading
    compiled contracts from disk and inspect or deploy them.

## Preview4:

```
epicchain-cli: v3.0.0-preview4
epicvault-js: v5.0.0-next.5
```
...
`epicvault-core` and `epicvault-api` is now supported and usable for this preview stage.

- Misc

  - EpicChain claim is now more complicated, sending EpicChain to the same address does not
    give you EpicChain anymore. Currently, this function is not supported until
    further notice. If you want to claim epicchain, just have 2 addresses and send EpicChain
...
- CONST
  ...
  - Deprecate ASSET_ID in favor of NATIVE_CONTRACT_HASH. Asset as a term is
    pretty much deprecated in epicchain.

- experimental

  - new section under epicvault-js to test out a different way of doing things.
...
```
epicchain-cli v3.0.0-preview3
epicvault-js: v5.0.0-next.4
```
...
As usual, only `epicvault-core` will be updated and usable during the preview stage.
...
- rpc
  ...
  - Updated fields to use lowercased strings with no separators. This should not
    have any effect internally as epicvault-js will continue to use camelCase.

## Preview2:

```
epicchain-cli: v3.0.0-preview2
epicvault-js: v5.0.0-next.3
```
...
Due to the large number of changes ongoing, only `epicvault-core` will be fully
...
- tx
  ...
  - Add support for reading JSON outputs from epicchain RPC endpoints.
  -
...
```
epicchain-cli: v3.0.0-preview1
epicvault-js: v5.0.0-next.2
```

Key blockchain changes:

- Removal of the UTXO system coupled with the integration of additional RPC
  methods on the epicchain node allowed for the wallet to operate without the need for
  a third party aggregator. EpicVault-api is now a high level API package.

- A change in the epicchainVM and the generation of scripts means a change in
  addresses. Private and public keys are the only things that remain unchanged
  from epicchain. All other details (Scripthash, WIF, XEP2, Address) are different.

SDK changes:

- EpicVault-api

  - Removed all provider modules. We do not need to rely on epicscan for details
...
- epicvault-js
  ...
  - Updated networks to connect to epicchain TestNet.
