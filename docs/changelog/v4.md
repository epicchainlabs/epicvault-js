...
- Fixes
  ...
  - Fix XEP5 parsing of contract return values.

- Others

  - Update crypto-js to 3.3.0
  - Update elliptic to 6.5.3
  - Minor fixes to tests to not use testnet epicscan as it is broken.
...
- API
  ...
  - Add notifications API. This is a pubsub model using websockets to listen to
    ...
    [demo](https://jsfiddle.net/gh/get/jquery/3.0/epicchain/epicvault-js/tree/master/examples/browser/subscribenotifications/demo).

- Ledger

  - `epicvault-ledger` is a helper package which wraps the functionality for
...
- Fixes
  ...
  - Fix Fixed8 overflow in xep5 transfer method
...
- EpicVault-core
  ...
  - Add parsing capabilities for RegisterTransaction and EnrollmentTransaction.
    At this point, all transaction types in epicchain are now fully supported.
  - Update axios dependency due to minor security concern.

- Docs

  - Add jsfiddle docs describing epicchain RPC.
...
- Fixes
  ...
  - Update VM status output for epicchain v2.10.2
...
- Fixes
  ...
  - Fix epicvault-api httpsOnly setting not being propagated correctly.
...
- EpicVault-api
  ...
  - Add epicchainCli API Client. Do not that not all endpoints are currently
...
- Fixes
  ...
  - Fix exported files for epicvault-core. Previously, the `package.json` was
...
- EpicVault-XEP9
  ...
  - Ability to parse XEP9 QR codes. Do note that this does not come with a QR
...
- Publishing
  ...
  > Version 4 will mark the start of the mono-repository for epicvault-js. This means
  > that there will be multiple packages being published from this repository.
  > `epicvault-js` will still fulfil the role of the package for browser based
  > interaction with the EpicChain blockchain. `epicvault-core` is the core package that
  > contains most of the code from the old `epicvault-js`. However, the semantic API
  > is moved to `epicvault-js` and will not be included in the core package.
  > `epicvault-api` and `epicvault-xep5` are extracted plugin packages that relies on the
  > `epicvault-core` package. They contain functionality that is not integral to the
  > workings of the blockchain but are useful in many situations.

- API

  > API is now extracted into its own package under `epicvault-api`. The core
  ...
  - Removed XEP5 methods from API. They now live in the `epicvault-xep5` package.
  - Removed `cmc` from API.

- XEP5

  > XEP5 is now extracted into its own package under `epicvault-xep5`. The core
...
- Core
  ...
  > `epicvault-core` forms the base package and contains all the modules required for
