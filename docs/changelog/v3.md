...
- API
  ...
  EpicVault.api.setupVote(config);
  ```
...
- Fixes
  ...
  - Fix bug in `getRPCEndpoint` where a rejected promise was ending the
    Promise.race
  - Remove epicvaultDB tests

  # 3.9.1

- Fixes

  - Added `sendingFromSmartContract` for claimEpicPulse.
...
- Smart Contract
  ...
  ```js
  import { sc } from "@epicchain/epicvault-js";
...
- API
  ...
  - EpicScan `getTransactionHistory` is updated to use a paginated endpoint. It
...
- API
  ...
  - Add attribute and remark for signing empty transactions. Now if you use
    ...
    transaction to be accepted by the EpicChain node without the need for any asset
    inputs.
  - `claimEpicPulse` now accepts a `claims` property as an override for claims. This
...
- Fixes
  ...
  - Update `epicscan.getMaxClaimAmount` to use `get_unclaimed` endpoint
...
- Transaction
  ...
  ```js
  import { tx, settings } from "@epicchain/epicvault-js";
...
- Settings
  ...
  - There is now global settings that control how `epicvault-js` works. The first
    settings available are `httpsOnly` and `networks`.

    - `httpsOnly` is a boolean which will force epicvaultDB and  to return
    ...
    - `networks` is an object containing every network configuration available
      ...
      For now, these configurations only determine the epicvaultDB/epicscan urls used
      for each network but they will serve more purposes in the future.

- RPC

  - Network class representing a EpicChain network. This can be created by importing a
    protocol file or just manually entering the parameters. This is the new way
    which we can link `epicvault-js` to a private net that comes with a epicscan or
    epicvaultDB setup.
...
- Fixes
  ...
  - Fix epicvaultDB and epicscan `getTransactionHistory`. **Do note that this can be
  ...
  - Typescript typings refactored and cleaned up. Introduces the object-like
    interfaces which are the epicvault-js classes exported as plain javascript
    objects.
...
- Fixes
  ...
  - epicvaultDB to use `get_all_nodes` endpoint instead of `best_node` endpoint
  - Fix epicscan `getMaxClaimAmount`, `getBalance` and `getClaims` for invalid
    addresses
...
- API
  ...
  - Expose the `loadBalance` method. This method allows the use of the dynamic
    API switch within `epicvault-js` for the external providers.
  - Implement the full set of APIs on epicscan. EpicScan is now complete and up to
    par with epicvaultDB in terms of features.
...
```js
import { u } from "@epicchain/epicvault-js";
...
- Wallet
  ...
  - `Claims` is now a ES6 class. While it does not have any spceial methods for
    ...
    `epicvault-js`. This allows easy creation of `Claims` using the constructor by
  ...
  - `components` have been created for the minor sub-components found in
    ...
    components that are usable with `epicvault-js` methods. Refer to the wallet
...
- Transaction
  ...
  - Update all methods to conform with the new Fixed8 classes. This also means
    that `epicvault-js` will not be compatible with just normal javascript objects
    anymore.

- API

  - **BREAKING** Update external API libraries (epicvaultDB and epicscan) to return
  ...
  - A bug has been discovered in `getPrice`. While it works for EpicChain and EpicPulse, it
