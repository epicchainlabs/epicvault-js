...
const contractHash = CONST.NATIVE_CONTRACT_HASH.EpicChain;
let txid: string;
...
describe("EpicChainServerRpcClient", () => {
  ...
  test("getNativeContracts", async () => {
    ...
    expect(contractNames).toContain("EpicChain");
    expect(contractNames).toContain("EpicPulse");
  ...
  describe.only("findStorage", () => {
    test("find EpicChain totalSupply storage", async () => {
      ...
      // This find will only get the totalSupply of EpicChain. Should be safe and static for usage.
    ...
    test("find EpicChain balance storage", async () => {
      // This find will only get a list of accounts on EpicChain storage. Will only be used to check the truncated and next values.
      ...
      // There isn't 50 or more accounts with EpicChain on this blockchain instance.
  ...
  describe("Invocation methods", () => {
    ...
    test.skip("invokeContractVerify", async () => {
      ...
      expect(result).toMatchObject({
        ...
        state: "HALT",
        epicpulseconsumed: expect.any(String),
        exception: null,
    ...
    test("invokeScript", async () => {
      ...
      expect(result.stack[0].value).toEqual(
        u.HexString.fromAscii("EpicChain").toBase64()
      );
    });
  ...
  test("getUnclaimedEpicPulse", async () => {
    const result = await client.getUnclaimedEpicPulse(
  ...
  test.only("traverseIterator", async () => {
    const newClient = new rpc.EpicChainServerRpcClient(
      "https://testnet3-seed.epic-chain.org:20111"
    );
